  ...
  return (
    <div className="min-w-[850px] bg-[#f5f5f5] p-4 text-[12px] text-[#161616] dark:bg-[#171717] dark:text-gray-100 sm:p-8">
      <div className="mx-auto max-w-[1420px] overflow-hidden rounded-md border border-[#b7c6d1] bg-white shadow-2xl shadow-black/10 ring-1 ring-black/[0.03] dark:border-gray-700 dark:bg-[#202020] dark:shadow-black/50 dark:ring-white/5">
        <div className="flex items-end border-b border-[#9caebc] bg-[#fafafa] px-3 pt-3 shadow-sm dark:border-gray-700 dark:bg-[#252525]">
          <button onClick={() => setTab('general')} className={cn('rounded-t-sm border border-b-0 px-5 py-2.5 text-[14px] font-medium transition-colors', tab === 'general' ? 'border-[#9caebc] bg-[#064b82] text-white shadow-sm' : 'bg-[#eeeeee] text-[#084579] hover:bg-[#e2e2e2] dark:bg-[#333] dark:hover:bg-[#3a3a3a]')}>General</button>
          <button onClick={() => setTab('detalle')} className={cn('rounded-t-sm border-b-0 px-3 py-2.5 text-[11px] font-semibold transition-colors', tab === 'detalle' ? 'border border-b-0 bg-white text-[#064b82] shadow-sm dark:bg-[#202020]' : 'text-[#064b82] hover:text-[#043457]')}>Detalle</button>
          <div className="ml-3 flex items-center gap-2 border-l border-[#aaa] pb-2.5 pl-3 text-[13px] font-bold">Saldo Anterior <span className="ml-1 rounded-sm border border-[#c4c4c4] bg-white px-4 py-1.5 font-mono font-normal text-blue-700 shadow-inner shadow-black/5 dark:bg-[#292929]">{money(saldoAnterior)}</span></div>
          <div className="ml-4 hidden items-center gap-2 pb-2.5 font-bold md:flex">Cuadre de Caja No. <span className="rounded-sm border border-[#c9c9c9] bg-white px-4 py-1.5 font-mono text-red-500 shadow-inner shadow-black/5 dark:bg-[#292929]">00000641</span></div>
        ...
        {tab === 'general' ? <>
          ...
          <div className="grid gap-3 border-b border-[#aaa] px-4 pb-3 md:grid-cols-[1fr_420px]"><label className="flex items-center gap-3 font-bold uppercase tracking-wide">Fecha último cuadre caja <span className="w-full rounded-sm border border-[#c4c4c4] bg-white px-2 py-1.5 text-right font-normal normal-case shadow-inner shadow-black/5 dark:bg-[#292929]">domingo, 21 de junio de 2026</span></label><div className="flex items-center justify-center gap-3"><span>Centro <b className="rounded-sm border border-[#c4c4c4] bg-white px-4 py-1.5 font-normal shadow-inner shadow-black/5 dark:bg-[#292929]">002</b></span><span>Cuenta <b className="rounded-sm border border-[#c4c4c4] bg-white px-4 py-1.5 font-normal shadow-inner shadow-black/5 dark:bg-[#292929]">11050502</b></span><b className="rounded-sm border border-[#c4c4c4] bg-white px-4 py-1.5 font-normal shadow-inner shadow-black/5 dark:bg-[#292929]">9002</b></div></div>
          <div className="flex justify-end px-4 py-3"><span className="w-[165px] rounded-sm bg-red-600 py-1.5 text-center font-semibold text-white shadow-sm">Periodo Cerrado</span></div>
          <div className="overflow-x-auto px-4 pb-3"><table className="w-full min-w-[760px] border-collapse overflow-hidden rounded-sm border border-[#151515] shadow-sm"><thead><tr className="bg-[#064b82] text-white"><th className="w-[30%] border border-[#777] px-2 py-2 font-medium tracking-wide">Descripción o concepto</th>{['Efectivo', 'Tarjetas', 'Consignado', 'Total', 'Anticipo x Dev'].map(h => <th key={h} className="border border-[#777] px-2 py-2 font-medium tracking-wide">{h}</th>)}</tr></thead><tbody>{lines.map((line, rowIndex) => <tr key={line.label} className={cn('transition-colors', line.kind === 'income-total' || line.kind === 'expense-total' ? 'bg-[#c8def2] font-bold' : line.kind === 'flow' ? 'bg-[#dcebf7] font-bold' : rowIndex === 0 ? 'bg-[#d7e9f8] text-blue-800 hover:bg-[#cbe0f4]' : 'hover:bg-[#f3f7fb] dark:hover:bg-[#252525]')}><td className="border border-[#c4c4c4] px-2 py-1.5">{line.label}</td>{line.values.map((value, colIndex) => <td key={`${line.label}-${colIndex}`} className="border border-[#c4c4c4] px-2 py-1.5 text-right font-mono">{rowIndex === 0 && colIndex === 0 ? <BoardInput value={value} onChange={setVentasEfectivo} /> : rowIndex === 0 && colIndex === 1 ? <BoardInput value={value} onChange={setVentasTarjetas} /> : rowIndex === 0 && colIndex === 2 ? <BoardInput value={value} onChange={setVentasConsignado} /> : rowIndex === 1 && colIndex === 2 ? <BoardInput value={value} onChange={setRecaudos} /> : money(value)}</td>)}<td className="border border-[#c4c4c4] px-2 py-1.5 text-right font-mono">{money(line.values.reduce((sum, n) => sum + n, 0))}</td><td className="border border-[#c4c4c4] px-2 py-1.5 text-right font-mono">0.00</td></tr>)}</tbody></table></div>
          <div className="grid gap-3 px-6 pb-4 pt-2 lg:grid-cols-2"><label className="flex items-center gap-2">Responsable Caja <span className="flex-1 rounded-sm border border-[#c4c4c4] bg-white px-2 py-1.5 text-blue-800 shadow-inner shadow-black/5 dark:bg-[#292929]">PEREZ POLANCO ANDRES ARTURO</span></label><label className="flex items-center gap-2">Cédula <span className="rounded-sm border border-[#c4c4c4] bg-white px-3 py-1.5 font-mono shadow-inner shadow-black/5 dark:bg-[#292929]">1140848342</span></label></div>
          <div className="grid items-end gap-4 border-t border-[#aaa] px-6 py-4 lg:grid-cols-[160px_160px_160px_1fr]">{[['Disponible En Caja Según Sistema', disponible], ['Efectivo según Arqueo de Caja', 0], ['Sobrante/Faltante En caja', 0]].map(([label, value], index) => <label key={label as string} className={cn('text-[10px] font-bold tracking-wide', index === 2 ? 'text-red-700' : '')}>{label as string}<span className="mt-1.5 block rounded-sm border border-[#aaa] bg-white px-2 py-2.5 text-right font-mono text-[14px] font-bold text-[#064b82] shadow-sm dark:bg-[#292929]">{money(value as number)}</span></label>)}<div className="flex justify-end gap-4 rounded-sm border border-[#aaa] p-3 text-lg shadow-sm">{['Imprimir', 'Imprimir', 'Exportar', 'Abrir'].map((title, i) => <span key={i} title={title} className="cursor-pointer transition-transform hover:scale-110 hover:text-[#064b82]">{['▣', '▣', '▧', '▱'][i]}</span>)}</div></div>
