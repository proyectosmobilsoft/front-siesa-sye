...
export const ReciboCajaPage = () => {
  ...
  return (
    <div className="min-w-[850px] bg-[#f5f5f5] p-3 text-[12px] text-[#161616] dark:bg-[#171717] dark:text-gray-100 sm:p-5">
      <div className="mx-auto max-w-[1420px] border border-[#b7c6d1] bg-white shadow-lg shadow-black/5 dark:border-gray-700 dark:bg-[#202020] dark:shadow-black/40">
        <div className="flex items-end border-b border-[#9caebc] bg-[#fafafa] px-2 pt-2 dark:border-gray-700 dark:bg-[#252525]">
          <button onClick={() => setTab('general')} className={cn('border border-b-0 px-5 py-2 text-[14px] transition-colors', tab === 'general' ? 'border-[#9caebc] bg-[#064b82] text-white' : 'bg-[#eeeeee] text-[#084579] hover:bg-[#e2e2e2] dark:bg-[#333] dark:hover:bg-[#3a3a3a]')}>General</button>
          <button onClick={() => setTab('detalle')} className={cn('border-b-0 px-2 py-2 text-[11px] font-semibold transition-colors', tab === 'detalle' ? 'border border-b-0 bg-white text-[#064b82] dark:bg-[#202020]' : 'text-[#064b82] hover:text-[#043457]')}>Detalle</button>
        ...
        {tab === 'general' ? <>
          <div className="grid items-end gap-x-3 gap-y-2 px-3 py-3 md:grid-cols-[315px_300px_1fr]">
            <label className="flex items-center gap-2">Caja<select value={caja} onChange={e => setCaja(e.target.value)} className="h-7 min-w-0 flex-1 border border-[#b9b9b9] bg-white px-1 transition-colors focus:border-[#064b82] focus:outline-none dark:bg-[#292929]"><option>CAJA SUCURSAL PORTAL DE SOLEDAD</option><option>CAJA PRINCIPAL</option></select></label>
            <label className="flex items-center gap-2">Fecha<div className="flex"><Input type="date" value={fecha} onChange={e => setFecha(e.target.value)} className="h-7 rounded-none border-[#b9b9b9] px-1 text-[12px] transition-colors focus-visible:border-[#064b82] focus-visible:ring-0" /><span className="flex h-7 items-center border border-l-0 border-[#b9b9b9] px-1">▦</span></div></label>
            <Button className="h-7 w-[120px] rounded-none border border-[#275e8b] bg-[#d8e7f1] px-2 text-[12px] font-normal text-[#111] transition-colors hover:bg-[#c6dce9] active:bg-[#b6d3e4]">🔍 Consultar</Button>
          </div>
          ...
          <div className="overflow-x-auto px-3 pb-2"><table className="w-full min-w-[760px] border-collapse border border-[#151515]"><thead><tr className="bg-[#064b82] text-white"><th className="w-[30%] border border-[#777] px-1.5 py-1.5 font-normal">Descripción o concepto</th>{['Efectivo', 'Tarjetas', 'Consignado', 'Total', 'Anticipo x Dev'].map(h => <th key={h} className="border border-[#777] px-1.5 py-1.5 font-normal">{h}</th>)}</tr></thead><tbody>{lines.map((line, rowIndex) => <tr key={line.label} className={cn('transition-colors', line.kind === 'income-total' || line.kind === 'expense-total' ? 'bg-[#c8def2] font-bold' : line.kind === 'flow' ? 'bg-[#dcebf7] font-bold' : rowIndex === 0 ? 'bg-[#d7e9f8] text-blue-800 hover:bg-[#cbe0f4]' : 'hover:bg-[#f3f7fb] dark:hover:bg-[#252525]')}><td className="border border-[#c4c4c4] px-1.5 py-1">{line.label}</td>{line.values.map((value, colIndex) => <td key={`${line.label}-${colIndex}`} className="border border-[#c4c4c4] px-1.5 py-1 text-right font-mono">{rowIndex === 0 && colIndex === 0 ? <BoardInput value={value} onChange={setVentasEfectivo} /> : rowIndex === 0 && colIndex === 1 ? <BoardInput value={value} onChange={setVentasTarjetas} /> : rowIndex === 0 && colIndex === 2 ? <BoardInput value={value} onChange={setVentasConsignado} /> : rowIndex === 1 && colIndex === 2 ? <BoardInput value={value} onChange={setRecaudos} /> : money(value)}</td>)}<td className="border border-[#c4c4c4] px-1.5 py-1 text-right font-mono">{money(line.values.reduce((sum, n) => sum + n, 0))}</td><td className="border border-[#c4c4c4] px-1.5 py-1 text-right font-mono">0.00</td></tr>)}</tbody></table></div>
          ...
          <div className="grid items-end gap-3 border-t border-[#aaa] px-5 py-3 lg:grid-cols-[160px_160px_160px_1fr]">{[['Disponible En Caja Según Sistema', disponible], ['Efectivo según Arqueo de Caja', 0], ['Sobrante/Faltante En caja', 0]].map(([label, value], index) => <label key={label as string} className={cn('text-[10px] font-bold', index === 2 ? 'text-red-700' : '')}>{label as string}<span className="mt-1 block border border-[#aaa] bg-white px-2 py-2 text-right font-mono text-[14px] font-bold text-[#064b82] shadow-sm dark:bg-[#292929]">{money(value as number)}</span></label>)}<div className="flex justify-end gap-4 border border-[#aaa] p-3 text-lg">{['Imprimir', 'Imprimir', 'Exportar', 'Abrir'].map((title, i) => <span key={i} title={title} className="cursor-pointer transition-transform hover:scale-110 hover:text-[#064b82]">{['▣', '▣', '▧', '▱'][i]}</span>)}</div></div>
